...
describe("captureFingerprint", () => {
  test("runs named cases in order and records project-defined probes", async () => {
    ...
    expect(artifact.cases).toEqual([
      { name: "start", probe: { animation: "none", stable: "yes", value: "0" } },
      { name: "stepped", probe: { animation: "none", stable: "yes", value: "1" } },
    ]);
  });
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        expect(artifact.cases[0]).toEqual({ name: "ready", probe: { text: "ready" } });
